  ...
  return (
    <form onSubmit={handleSubmit} className="mt-4 grid max-w-md gap-3">
      ...
      <label className="grid gap-1">
        Ditt namn
        <input value={fields.reviewerName} onChange={set("reviewerName")} required className="rounded border p-2" />
      </label>
      <label className="grid gap-1">
        Betyg
        <select value={fields.rating} onChange={set("rating")} className="rounded border p-2">
          {[5, 4, 3, 2, 1].map((n) => (
      ...
      <label className="grid gap-1">
        Kommentar
        <textarea value={fields.comment} onChange={set("comment")} required rows={3} className="rounded border p-2" />
      </label>
